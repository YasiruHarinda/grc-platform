...
func (s *controlService) DeleteControl(ctx context.Context, auditID, controlID int) error {
	...
	// AUDIT_MANAGE_CONTROLS-gated (see the outer gateway's RequirePrivilege
	// check) — an admin's delete is intentionally unconditional, even once
	// evidence/population work exists; audit_evidence and audit_population
	// both cascade-delete with the control at the DB level (see audit_schema.sql).
