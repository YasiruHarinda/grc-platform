...
type FrameworkService interface {
	ListFrameworks(ctx context.Context, scope model.Scope, userEmail string) ([]*model.AuditFramework, error)
...
func (s *frameworkService) ListFrameworks(ctx context.Context, scope model.Scope, userEmail string) ([]*model.AuditFramework, error) {
	return s.frameworkRepo.List(ctx, scope, userEmail)
}
