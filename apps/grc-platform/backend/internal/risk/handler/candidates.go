...
// handleListManagementApprovers serves GET /api/v1/risks/management-approvers:
...
// handleListRiskOwnerCandidates serves GET /api/v1/risks/owner-candidates:
...
// handleListRiskAssignerCandidates serves GET /api/v1/risks/assigner-candidates:
...
func (d *Deps) resolveCandidates(ctx context.Context, candidates []idUUID, keepUnresolved bool) []*userentity.User {
	...
	for _, c := range candidates {
		...
		// did that — but the response shape is shared with GET /api/v1/risks/users, whose
